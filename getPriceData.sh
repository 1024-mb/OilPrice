...
if [[ "$result" != 0 ]] || echo "$response" | grep -q "Sorry - the page you tried to reach is no longer here." || echo "$response" | grep -q "Error"; then
...
# decreases data redundancy - by deleting all records that are no longer usable
/usr/bin/mysql -u "moiz" -p"${MYSQLPASS}" "CW_1314" <<EOFMYSQL
DELETE FROM CW_1314.OILPRICES WHERE DateReading < '${date}';
EOFMYSQL
